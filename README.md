...
[x] Create an index displaying Pokemon 'cards'
  [x] Render each Pokemon name, sprite, and hp in a card
  [x] _When clicked_, the card should toggle between displaying the front and back
    sprites
[x] Allow users to search a Pokemon by its name in order to narrow down the cards
  shown on the page
[x] Wire up the form to add a missing Pokemon (Bulbasaur is missing, and you can
